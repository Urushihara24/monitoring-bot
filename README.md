...
Важно по UX:
- кнопки `⬅/➡` только переключают **активный товар для управления**
  и оставляют пользователя в главном меню (без автоперехода в настройки)
...
- `активный товар` нужен для редактирования его настроек:
  режим, автоцена, лимиты, интервал, URL конкурента
...
- `⏱ Интервал`
- `🔀 Режим`
- `🔗 Добавить URL` / `🗑 Удалить URL` — для активного товара
...
- `📉 Мин` (`MIN_PRICE`)
- `📈 Макс` (`MAX_PRICE`)
- `📍 Позиция` (`POSITION_FILTER_ENABLED`)

### Режимы цены (простыми словами)
- `Следование`:
  ставим ровно цену конкурента (4 знака), например `0.3560 -> 0.3560`
- `Демпинг`:
  считаем от витринной цены конкурента (2 знака):
  `round(конкурент, 2) - 0.0051`, например витрина `0.35 -> 0.3449`
- `Повышение`:
  также от витринной цены (2 знака):
  `round(конкурент, 2) + 0.0049`, например витрина `0.35 -> 0.3549`
...
- Команды в редакторе:
  ...
  - `reset` — удалить все правила товара
