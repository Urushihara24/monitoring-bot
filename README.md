...
- `MIN_PRICE`, `MAX_PRICE`
- `MODE=FIXED|STEP_UP|FOLLOW_EXACT|FOLLOW_PLUS`
- `FOLLOW_EXACT`: ставить ровно цену конкурента
- `FOLLOW_PLUS`: ставить цену конкурента (витрина, 2 знака) + `FOLLOW_PLUS_VALUE`
- `MAX_DOWN_STEP` для ограничения резкого падения
- `FAST_REBOUND_DELTA` + bypass cooldown для быстрого отката вверх
- при `POSITION_FILTER_ENABLED=true` и `rank=N/A` включается эвристика
  `WEAK_UNKNOWN_RANK_*` (абсолютный/относительный gap между 1-й и 2-й ценой),
  чтобы не демпинговать за "слабым" конкурентом
...
Настройки:
- `🔔 Авто: ВКЛ/ВЫКЛ` (автоценообразование для активной площадки)
- `⬆ +0.01₽` / `⬇ -0.01₽` (ручная корректировка)
- `🎯 Цена` (`DESIRED_PRICE`)
- `➖ Шаг` (`UNDERCUT_VALUE`)
- `📉 Мин` (`MIN_PRICE`)
- `📈 Макс` (`MAX_PRICE`)
- `📦 Товары` — управление списком товаров мониторинга:
  отправьте просто `product_id` (добавить/выбрать), для списка — `list`
  или сразу пару `product_id url_конкурента`
- `⏱ Интервал` (`CHECK_INTERVAL`)
- `🔀 Режим` (циклически):
  `Фиксированный порог` → `Шаг вверх от текущей` →
  `Следовать: точно как конкурент` → `Следовать: витрина + 0.0049`
- `📍 Позиция`
- `💬 Инструкции: ВКЛ/ВЫКЛ` (только для профилей с chat API)
- `🔗 Добавить URL` / `🗑 Удалить URL`
- `🧾 История`

Важно:
- один профиль может мониторить несколько товаров одновременно
- для каждого товара хранится свой список URL конкурентов
- после выбора товара через `📦 Товары` кнопка `🔗 Добавить URL`
  добавляет конкурента именно для выбранного товара
- в статусе выводится список пар `мой товар ↔ URL конкурента`
- при добавлении/удалении товара через Telegram новый список scheduler’ов
  применяется после перезапуска процесса/контейнера
